    ...
    while True:
        try:
            ...
            if image_path and os.path.exists(image_path):
                ...
                msg.set_content(
                    f'Intruder detected at {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'
                )
...
# ================= DISCORD WORKER =================
def send_discord_alert_async(image_path, message="Intruder detected!"):
    def send():
        try:
            if image_path and os.path.exists(image_path):
...
def main():
    ...
    while True:
        ...
        intruder = (
            motion_pixels > 500
            and
            (len(faces) > 0 or person_detected)
        )
        ...
        if intruder and recording:
            ...
            if now - last_telegram_snapshot_time >= TELEGRAM_COOLDOWN:
                ...
                send_telegram_photo_async(
                    temp_snap,
                    f"⚠️ Intruder still present - {timestamp}"
                )
            ...
            # DISCORD
            if now - last_discord_snapshot_time >= DISCORD_COOLDOWN:

                temp_snap = (
                    f"snapshots/discord_temp_"
                    f"{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
                )
