...
if face_cascade.empty():
    print("âŒ ERROR: Haarcascade not loaded")
    exit()

# ================= HOG PERSON DETECTOR (ADDED) =================
...
def upload_to_gdrive(file_path):
    try:
        ...
        print("â˜ï¸ Uploaded to Drive (INTRUDER DETECTED):", file.get('id'))
...
def start_recording(frame):
    ...
    threading.Thread(target=send_telegram, args=(snap, "ðŸš¨ INTRUDER")).start()
...
def main():
    ...
    print("ðŸŒ Live Stream URL:", STREAM_URL)
    ...
    while True:
        ...
        # Non-Max Suppression (reduce duplicates)
        pick = []
        if len(boxes) > 0:
        ...
        # ===== FINAL INTRUDER LOGIC =====
        intruder = (len(faces) > 0) or person_detected
